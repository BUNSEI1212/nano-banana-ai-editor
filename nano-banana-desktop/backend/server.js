...
// API Key管理端点（仅限管理员）
app.get('/admin/api-keys/stats', (req, res) => {
  try {
    const stats = geminiService.apiKeyManager.getStats();
...
app.post('/admin/api-keys/add', async (req, res) => {
  try {
    const { key, name, maxQPM, maxDaily, priority } = req.body;

    if (!key || !name) {
      return res.status(400).json({ error: 'Key and name are required' });
    }

    await geminiService.apiKeyManager.addApiKey({
      key,
      name,
      maxQPM: maxQPM || 60,
      maxDaily: maxDaily || 1000,
      priority: priority || 1
    });
...
app.delete('/admin/api-keys/:name', async (req, res) => {
  try {
    const { name } = req.params;
    await geminiService.apiKeyManager.removeApiKey(name);
...
app.post('/api/generate', authMiddleware, quotaMiddleware, async (req, res) => {
  try {
    const { prompt, referenceImages = [], refImages = [], options = {} } = req.body;
    // Support both referenceImages (frontend) and refImages (legacy)
    const images = referenceImages.length > 0 ? referenceImages : refImages;
    const userId = req.user.sub;
    ...
    if (error.type === 'BAD_REQUEST') {
      ...
      return res.status(400).json({
        ...
        details: 'The request was invalid or contained unsupported content'
      });
    }
